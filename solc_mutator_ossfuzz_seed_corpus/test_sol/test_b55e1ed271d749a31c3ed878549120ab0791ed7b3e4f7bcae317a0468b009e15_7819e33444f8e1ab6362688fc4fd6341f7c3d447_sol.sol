==== Source:  ====

==== Source: su0.sol ====
contract C0 {
  error er0();
  fallback() external virtual  
  {
    assert((payable(address(this)) < payable(address(this))));
    string storage l0;
    {
      bytes25 l1 = bytes25(0xffffffffffffffffffffffffffffffffffffffffffffffffff);
    }
  }
  bool  public s0;
  uint248   s1 = uint248(0);
  int248   s2;
  constructor(bool i0,int248 i1) payable  {
    s0 = false;
    s2 &= int248(226156424291633194186662080095093570025917938800079226639565593765455331327);
    { }
  }
}
pragma solidity >= 0.0.0;
enum EN0 {
  M0, M1, M2, M3, M4, M5, M6, M7,
  M8, M9, M10, M11, M12, M13, M14, M15,
  M16, M17, M18, M19, M20, M21, M22, M23,
  M24, M25, M26, M27, M28, M29, M30, M31,
  M32, M33, M34, M35, M36, M37, M38, M39,
  M40, M41, M42, M43, M44, M45, M46, M47,
  M48, M49, M50, M51, M52, M53, M54, M55,
  M56, M57, M58, M59, M60, M61, M62, M63,
  M64, M65, M66, M67, M68, M69, M70, M71,
  M72, M73, M74, M75, M76, M77, M78, M79,
  M80, M81, M82, M83, M84, M85, M86, M87,
  M88, M89, M90, M91, M92, M93, M94, M95,
  M96, M97, M98, M99, M100, M101, M102, M103,
  M104, M105, M106, M107, M108, M109, M110, M111,
  M112, M113, M114, M115, M116, M117, M118, M119,
  M120, M121, M122, M123, M124, M125, M126, M127,
  M128, M129, M130, M131, M132, M133, M134, M135,
  M136, M137, M138, M139, M140, M141, M142, M143,
  M144, M145, M146, M147, M148, M149, M150, M151,
  M152, M153, M154, M155, M156, M157, M158, M159,
  M160, M161, M162, M163, M164, M165, M166, M167,
  M168, M169, M170, M171, M172, M173, M174, M175,
  M176, M177, M178, M179, M180, M181, M182, M183,
  M184, M185, M186, M187, M188, M189, M190, M191,
  M192, M193, M194, M195, M196, M197, M198, M199,
  M200, M201, M202, M203, M204, M205, M206, M207,
  M208, M209, M210, M211, M212, M213, M214, M215,
  M216, M217, M218, M219, M220, M221, M222, M223,
  M224, M225, M226, M227, M228, M229, M230, M231,
  M232, M233, M234, M235, M236, M237, M238, M239,
  M240, M241, M242, M243, M244, M245, M246, M247,
  M248, M249, M250, M251, M252, M253
}
error er1();

==== Source: su1.sol ====
pragma solidity >= 0.0.0;
contract C1 {
  bool   s3;
  bytes18  public s4 = bytes18(0x000000000000000000000000000000000000);
  bool   s5;
  constructor(bool i0,bool i1)   {
    s3 = true;
    s5 = (((true ? false : (bytes12(0xffd14ce257b41169877a2339) < bytes12(0x000000000000000000000000))) ? uint232(1564624931424172144037071910106884047915805173658260058750306052185466) : uint232(2847080949833362884068851256978724937293407925986247333272266528087437)) > uint232(5775255180822420523022905724073655829375389354052931867142744954788779));
    unchecked {
      bool  l0 = s5;
      bool  l1 = l0;
      assert(l1 == s5);
      bool  l2 = s3;
      bool  l3 = l2;
      assert(l3 == s3);
      bool  l4 = s3;
      bool  l5 = l4;
      assert(l5 == s3);
      bool  l6 = s3;
      bool  l7 = l6;
      assert(l7 == s3);
      bool  l8 = s3;
      bool  l9 = l8;
      assert(l9 == s3);
      bool  l10 = s3;
      bool  l11 = l10;
      assert(l11 == s3);
    }
  }
  fallback() external virtual  
  {
    bool  l0 = s3;
    bool  l1 = l0;
    assert(l1 == s3);
  }
}
// ----
// Warning 2072: (su0.sol:132-149): Unused local variable.
// Warning 2072: (su0.sol:163-173): Unused local variable.
// Warning 5667: (su0.sol:326-333): Unused function parameter. Remove or comment out the variable name to silence this warning.
// Warning 5667: (su0.sol:334-343): Unused function parameter. Remove or comment out the variable name to silence this warning.
// Warning 5667: (su1.sol:152-159): Unused function parameter. Remove or comment out the variable name to silence this warning.
// Warning 5667: (su1.sol:160-167): Unused function parameter. Remove or comment out the variable name to silence this warning.
