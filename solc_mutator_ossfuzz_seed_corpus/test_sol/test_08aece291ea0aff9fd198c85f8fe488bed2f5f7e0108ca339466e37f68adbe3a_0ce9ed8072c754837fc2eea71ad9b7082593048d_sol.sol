==== Source:  ====

==== Source: su0.sol ====
contract C0 {
  address   s0;
  uint64   s1 = uint64(12102518104807600381);
  constructor(address i0)   {
    s0 = address(this);
    {
      (bool l0, bytes memory l1) = address(this).call(bytes("ffffffffffffffffff00000000000000"));
      address  l2 = s0;
      address  l3 = l2;
      assert(l3 == s0);
    }
  }
}
pragma solidity >= 0.0.0;
contract C1 {
  function f0() external virtual  
  {
  }
  enum EN0 {
    M0, M1, M2, M3, M4, M5, M6, M7,
    M8, M9, M10, M11, M12, M13, M14, M15,
    M16, M17, M18, M19, M20, M21, M22, M23,
    M24, M25, M26, M27, M28, M29, M30, M31,
    M32, M33, M34, M35, M36, M37, M38, M39,
    M40, M41, M42, M43, M44, M45, M46, M47,
    M48, M49, M50, M51, M52, M53, M54, M55,
    M56, M57, M58, M59, M60, M61, M62, M63,
    M64, M65, M66, M67, M68, M69, M70, M71,
    M72, M73, M74, M75, M76, M77, M78, M79,
    M80, M81, M82, M83, M84, M85, M86, M87,
    M88, M89, M90, M91, M92, M93, M94, M95,
    M96, M97, M98, M99, M100, M101, M102, M103,
    M104, M105, M106, M107, M108, M109, M110, M111,
    M112, M113, M114, M115, M116, M117, M118, M119,
    M120, M121, M122, M123, M124
  }
  C0   s2 = C0(address(this));
  C1.EN0   s3;
  constructor(C1.EN0 i0)   {
    s3 = C1.EN0(uint8(94));
    unchecked {
      C1.EN0  l0 = s3;
      C1.EN0  l1 = l0;
      assert(l1 == s3);
      (bool l2, bytes memory l3) = address(this).call(abi.encodeWithSignature("f0()"));
    }
  }
}
// ----
// Warning 5667: (su0.sol:90-100): Unused function parameter. Remove or comment out the variable name to silence this warning.
// Warning 2072: (su0.sol:143-150): Unused local variable.
// Warning 2072: (su0.sol:152-167): Unused local variable.
// Warning 5667: (su0.sol:1181-1190): Unused function parameter. Remove or comment out the variable name to silence this warning.
// Warning 2072: (su0.sol:1317-1324): Unused local variable.
// Warning 2072: (su0.sol:1326-1341): Unused local variable.
