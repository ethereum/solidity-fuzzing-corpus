...
contract C0 {
  ...
  address payable[2][][6]   s2;

	function compareMemoryAndStorage(address payable[2][][6] memory v1, address payable[2][][6] storage v2) internal returns (bool) {
		if (v1.length != v2.length)
			return false;
		for (uint i = 0; i < v1.length; i++)
  ...
  constructor(string memory i0,address payable[2][][6] memory i1)   {
    s1 = (true ? string("00ffffffffffffffffffffffffffff") : string("ffffffffffffffffffffffffffffffffffffffffffffffc477859b84eb16db4cb6cd41b1db3385126e5673"));
    s2 = i1;
    unchecked {
    }
  }
